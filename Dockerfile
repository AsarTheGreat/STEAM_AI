...
# Start Ollama, wait for it to be ready, pull llama3 if needed, then run the Python app
CMD ["bash", "-c", "ollama serve > /dev/null 2>&1 & until ollama list >/dev/null 2>&1; do sleep 1; done; if ! ollama list | grep -q '^llama3'; then echo 'Pulling llama3 model...'; ollama pull llama3; fi; echo 'Starting application...'; python main.py"]